 ...
 *
 * The rule joins the existing "User-agent: *" group instead of opening a second
 * one. Google merges repeated groups for the same agent, but plenty of smaller
 * crawlers read the first matching group and stop, so a second group is a rule
 * some of them never see.
 *
...
function lookdog_out_robots( $rules ) {
	...
	foreach ( $lines as $i => $text ) {
		if ( preg_match( '~^\s*User-agent:\s*\*\s*$~i', $text ) ) {
...
add_action(
	...
	static function () {
		...
		// human. The daily total jumped from about 40 to 297 on the day the
		// crawlers found the redirect. Every one of those was a real request to
		// an affiliate URL produced by no buyer at all - which is what
		...
		// resolves for anything following it, the affiliate URL is never handed
		// out, and the visit is counted separately so the split stays visible
		// rather than silently disappearing.
		if ( lookdog_out_is_bot() ) {
			$bots = (array) get_option( 'lookdog_click_bot_days', array() );
			$key  = gmdate( 'Y-m-d' );

			$bots[ $key ] = ( isset( $bots[ $key ] ) ? (int) $bots[ $key ] : 0 ) + 1;
			if ( count( $bots ) > 120 ) {
				ksort( $bots );
				$bots = array_slice( $bots, -120, null, true );
			}
			update_option( 'lookdog_click_bot_days', $bots, false );
...
<div class="wrap lookdog-stats">
	 ...
	 * a click, so the all-time figure is inflated and cannot be repaired - there
	 * is no record of which of the older ones were real. Saying so on the screen
	 * is better than quietly restating a number that was never true.
	 */
